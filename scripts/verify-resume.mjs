...
async function verifyCanonicalAndHtml() {
  ...
  for (const route of routes) {
    ...
    check(!/facebook|instagram|whatsapp|wechat/i.test(html), `${route.pathname} contains personal social links.`);
...
async function verifyPdf(filename, expectedTitle) {
  ...
  for (const required of ["Chay Kun Wai (Jacky)", "jackychay@live.com", "Midea Group", "Backend Lead Engineer (Technical Lead)", "China office and customer sites", "Dec 2024 - Jul 2026", "Swisslog Malaysia Sdn. Bhd.", "Malaysia office and regional customer sites", "Feb 2020 - Dec 2024", "Open to frequent worldwide travel", "AI-Assisted WMS Troubleshooting Prototype", "Internal prototype - not released", "Prototype V1", "Planned V2", "retrieval-augmented generation (RAG)", "read-only database", "Java", "SQL", "Tunku Abdul Rahman College"]) {
    check(extracted.includes(required), `${filename} extracted text is missing: ${required}`);
  }
  for (const prohibited of ["QR code", "Date of Birth", "Birth Year", "Age:", "Facebook", "Instagram", "WhatsApp", "github.com", "production AI", "direct reports", "7+ years", "99.99%"] ) {
...
async function verifyPdfs() {
  ...
  const backend = path.join(ROOT, "assets", "download", "Kun-Wai-Chay-Backend-Lead-Engineer.pdf");
  for (const alias of ["Kun-Wai-Chay-Senior-Backend-Engineer.pdf", "Chay Kun Wai - Resume.pdf"]) {
    const aliasPath = path.join(ROOT, "assets", "download", alias);
    check(await exists(aliasPath), `${alias} backend PDF alias is missing.`);
    if (await exists(backend) && await exists(aliasPath)) check(await sha256(backend) === await sha256(aliasPath), `${alias} is not byte-identical to the canonical backend PDF.`);
  }
}
